...
last_csv='./config/last_dealflowalerts.csv'
...
def fetch_data(urls):
    ...
    for i in urls:
        try:
            ...
            print(city)
            source='dealflowalerts.com'
            try:
                revenue=driver.find_element(By.XPATH, '//h5[text()="MRR"]/following-sibling::p').text.strip()
            except:
                revenue='N/A'
            try:
            ...
            try:
                ...
                ebitda='N/A'
            try:
                decription=driver.find_element(By.XPATH,'(//h2)[1]/following-sibling::p').text
            except:
                decription='N/A'
            try:
            ...
            try:
                ...
                industry='N/A'
            try:
                ask=driver.find_element(By.XPATH,'//h5[text()="Sale Price"]/following-sibling::p').text
            except:
                ask='N/A'

            try:
                source_url=driver.find_element(By.XPATH,'//a[text()="Browse on source site"]').get_attribute('href')
            except:
                source_url=''
            ...
            data_tocsv=[title,city,state,country,driver.current_url,industry,source,decription,listedby_firm,listedby,phone,mail,ask,revenue,cashflow,inventory,ebitda,formatted_date,'',source_url]
...
def login(driver):
    driver.get("https://dealflowalerts.com/login")
    time.sleep(2)
    mail=driver.find_element(By.XPATH,'//label[text()="Email *"]/following-sibling::div//input[@type="email"]')
    mail.send_keys("mary@teddyholdings.com")
    time.sleep(2)
    pwd=driver.find_element(By.XPATH,'//label[text()="Password * "]/following-sibling::div//input[@type="password"]')
    pwd.send_keys("Teddy1539")
...
def main():
    ...
    driver=webdriver.Chrome(service=ChromeService(ChromeDriverManager().install()),options=options)
    login(driver)
    driver.get("https://dealflowalerts.com/searches/bWljcm9hY3FfNHc5YUFBcGF2TQ==")
    
    results=driver.find_elements(By.XPATH,'//a[text()="Check deal"]')
    for result in results:
        urls.append("https://dealflowalerts.com/deals/"+result.get_attribute('data-id'))
    return urls
