...
def fetch_data(urls):
    ...
    for i in urls:
        try:
            ...
            try:
                phone += "Ofice :"+ re.search(r"Office: \((\d{3})\) (\d{3})-(\d{4})", text).group()
            except AttributeError:
                phone = "Office number not found"

            try:
                direct_call_number = "|| Direct :"+re.search(r"Direct: \((\d{3})\) (\d{3})-(\d{4})", text).group()
            except AttributeError:
                direct_call_number = "Direct call number not found"
...
def main():
    ...
    while True:
        ...
        for result in results:
            try:
                cashflow=result.find_element(By.XPATH,'.//p[@class="financials"]').text.split("Cash Flow:\n")[1]
            except:
                cashflow=''
            if cashflow_revenue_checker(cashflow):
                url=result.find_element(By.XPATH,'.//a[text()="View Full Listing"]')
                urls.append("https://www.bizex.net"+url.get_attribute('href'))
        try:
