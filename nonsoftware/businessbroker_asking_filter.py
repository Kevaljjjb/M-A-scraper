...
def main():
    ...
    urls=[]
    for i in range(1,12):
        base_url=f'https://www.businessbroker.net/listings/bfs_result.aspx?By=AdvancedSearch&r_id=33&ind_id=0&ask_price_l=6000000&ask_price_h=&map_id=0&lcity=&keyword=&lst_no=&time=0&bprice=0&fresale=0&ownerfi=0&county_id=0&page={str(i)}'
        driver.get(base_url)
        time.sleep(3)
        try:
            wait.until(EC.presence_of_element_located((By.XPATH, '//a[text()="Read More"]')))
            wait.until(EC.visibility_of_element_located((By.XPATH, '//span[contains(text(),"Asking Price:")]')))
            results=driver.find_elements(By.XPATH,'//div[@class="result-item listing"]')
            for i in results:
                try:
                    asking=i.find_element(By.XPATH,'.//span[contains(text(),"Asking Price:")]').text
                    ...
                    cf=rev=asking=''
                if '$' not in cf and '$' not in rev and '$' in asking and asking_checker(asking):
