...
base_url = "https://fcbb.com/businesses-for-sale#SearchAvailableBusinesses"
...
def main_page(url,driver):
    ...
    driver.get(url)
    wait = WebDriverWait(driver, 20)
    wait.until(EC.presence_of_element_located((By.ID, 'inputSortBy')))
    time.sleep(4)
        ...
        try:
            for result in results:
                try:
                    ...
                    revenue=''
                    print(str(e))
                asking=result.find_element(By.XPATH,'.//p[contains(@class,"asking-price")]').text
                ...
                if asking_checker(asking):
                    if '$' not in revenue:
                        data.append(atag)
                else:
                    raise Exception("No more values above 6 Mil in asking price")
            try:
