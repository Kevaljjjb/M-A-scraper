...
base_url = "http://www.globalbx.com/results.asp?q=YTE9NjAwMDAwMCZibG9jPTAmdD0w"
...
def main_page(url,driver):
    ...
    driver.get(url)
    wait = WebDriverWait(driver, 20)
    wait.until(EC.presence_of_element_located((By.XPATH, '//table[@id="lt"]//tr//td[2]/a')))
    # time.sleep(4)
    ...
    data=[]
    while True:
        results = driver.find_elements(By.XPATH, '//table[@id="lt"]//tr/td[text()="United States"]/..')
        try:
            for result in results:
                asking=result.find_element(By.XPATH,'./td[4]').text
                cf=result.find_element(By.XPATH,'./td[5]').text
                rev=result.find_element(By.XPATH,'./td[6]').text
                
                if '$' not in cf and '$' not in rev and '$' in asking:
                    data.append(result.find_element(By.XPATH,'./td[2]/a').get_attribute("href"))
